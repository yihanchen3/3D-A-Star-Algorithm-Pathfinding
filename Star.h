...
public:
	...
	bool  Creatgraph(std::string input_file = "input.txt");

	bool  findpath(std::string input_file) 
	{
		return Creatgraph(input_file); 
	...
	void  animate(){
		...
		{
			cout << path[i].xPos << " " << path[i].yPos << " " << path[i].zPos << endl;
		}
	}
